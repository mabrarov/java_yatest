if (${env:COVERAGE_BUILD} -ne 0) {
  ...
  appveyor-retry codecov --required --token "${env:CODECOV_TOKEN}" --file "${rle_codecov_coverage_file}" --file "${jewellery_and_stones_codecov_coverage_file}" --root "${codecov_root_folder}" -X gcov;
